...
const openTuanCK = $.isNode() ? (process.env.OPEN_DREAMFACTORY_TUAN ? process.env.OPEN_DREAMFACTORY_TUAN : '0'):'0';
...
!(async () => {
  $.openTuanCKList = openTuanCK.split(',');
...
async function jdDreamFactory() {
  try {
	await userInfo();
	if($.openTuanCKList.includes($.index.toString())){
		await tuanActivity();
		await QueryAllTuan();
	}else{
		allMessage+= `此帐号不开团跳过\n`
		console.log(`此帐号不开团跳过`);
	}
  } catch (e) {
    $.logErr(e)
  }
...
async function joinLeaderTuan() {
  let res = await updateTuanIdsCDN('https://raw.githubusercontent.com/FearNoManButGod/AuthorCode/main/jd_dreamFactory_tuan.json')
  if (!res) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/jd_dreamFactory_tuan.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await updateTuanIdsCDN('https://cdn.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/jd_dreamFactory_tuan.json');
  }
...
function requireConfig() {
  return new Promise(async resolve => {
    ...
    if (!tuanActiveId) {
      await updateTuanIdsCDN('https://raw.githubusercontent.com/FearNoManButGod/AuthorCode/main/jd_dreamFactory_tuan.json');
      if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
        ...
        if (!$.tuanConfigs) {
          $.http.get({url: 'https://purge.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/jd_dreamFactory_tuan.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
          await $.wait(1000)
          await updateTuanIdsCDN('https://cdn.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/jd_dreamFactory_tuan.json');
          if ($.tuanConfigs && $.tuanConfigs['tuanActiveId']) {
            tuanActiveId = $.tuanConfigs['tuanActiveId'];
            console.log(`拼团活动ID: 获取成功 ${tuanActiveId}\n`)
          } else {
