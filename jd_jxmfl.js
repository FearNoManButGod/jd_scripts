...
if($.isNode()){
	Object.keys(jdCookieNode).forEach(_0x5c4b2e=>{
		cookiesArr.push(jdCookieNode[_0x5c4b2e]);
	});
	if(process.env.JD_DEBUG&&process.env.JD_DEBUG==='false')console.log=()=>{};
}else{
	cookiesArr=[$.getdata('CookieJD'),$.getdata('CookieJD2'),...jsonParse($.getdata('CookiesJD')||'[]').map(_0x41501a=>_0x41501a.cookie)].filter(_0x4ffbd2=>!!_0x4ffbd2);
}
...
!(async()=>{
	...
	await requestAlgo();
	const _0x4f448d=(Math.random()>0.5)?'https://kingran.coding.net/p/yq.json/d/shareCodes/git/raw/master/jxmfl.json':'https://kingran.coding.net/p/yq.json/d/shareCodes/git/raw/master/jxmfl.json';
	launchid=(await getAuthorShareCode(_0x4f448d))||[];
	if(process.env.launchid){
		launchid=process.env.launchid.split('@');
	}if((launchid.length==0)&&!first){
		return;
	}for(let _0x9b5747=0;_0x9b5747<cookiesArr.length;_0x9b5747++){
		if(cookiesArr[_0x9b5747]){
			cookie=cookiesArr[_0x9b5747];
	...
	$.log('','❌ '+$.name+', 失败! 原因: '+_0x3d8422+'!','');
}).finally(()=>{
	$.done();
});
async function help_all(){
	for(let _0xfdd97f=0;_0xfdd97f<launchid.length;_0xfdd97f++){
		$.signle_launchid=launchid[_0xfdd97f];
		await help();
		await $.wait(2000);
	}
}
function getAuthorShareCode(_0x4e99a0){
	return new Promise(_0x501540=>{
		const _0x412199={'url':_0x4e99a0+'?'+new Date(),'timeout':10000,'headers':{'User-Agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88'}};
		$.get(_0x412199,async(_0x3886e1,_0x15b4fb,_0x53969b)=>{
			try{
				if(_0x3886e1){
					$.getAuthorCodeListerr=false;
				}else{
					if(_0x53969b)_0x53969b=JSON.parse(_0x53969b);
					$.getAuthorCodeListerr=true;
				}
			}catch(_0x1cedf4){
				$.logErr(_0x1cedf4,_0x15b4fb);
				_0x53969b=null;
			}
			finally{
				_0x501540(_0x53969b);
...
async function taskPostUrl(_0x3b96bf,_0x2173eb){
	return{'url':''+JD_API_HOST,'body':'functionId='+_0x3b96bf+'&body='+escape(JSON.stringify(_0x2173eb))+'&client=wh5&clientVersion=1.0.0&appid=content_ecology&uuid=6898c30638c55142969304c8e2167997fa59eb54&t=1622588448365','headers':{
...
async function TotalBean(){
	return new Promise(async _0x5aa0a0=>{
		const _0x134968={'url':'https://wq.jd.com/user/info/QueryJDUserInfo?sceneval=2','headers':{
		...
		$.post(_0x134968,(_0x2a1a44,_0x4a85d1,_0x5cf31d)=>{
			try{
				if(_0x2a1a44){
					console.log(''+JSON.stringify(_0x2a1a44));
					console.log($.name+' API请求失败，请检查网路重试');
				}else{
					if(_0x5cf31d){
						_0x5cf31d=JSON.parse(_0x5cf31d);
						if(_0x5cf31d.retcode===13){
							$.isLogin=false;
							return;
						}if(_0x5cf31d.retcode===0){
							$.nickName=_0x5cf31d.base&&_0x5cf31d.base.nickname||$.UserName;
						}else{
							$.nickName=$.UserName;
						}
					}else{
...
async function safeGet(_0x3add25){
	try{
		if(typeof JSON.parse(_0x3add25)=='object'){
			return true;
		}
	}catch(_0x109c35){
		console.log(_0x109c35);
		console.log('京东服务器访问数据为空，请检查自身设备网络情况');
		return false;
	}
}
function jsonParse(_0x433fc0){
	if(typeof _0x433fc0=='string'){
		try{
			return JSON.parse(_0x433fc0);
		}catch(_0x2c4c02){
			console.log(_0x2c4c02);
