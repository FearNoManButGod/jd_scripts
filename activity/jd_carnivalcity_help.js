...
function shareCodesFormat() {
  return new Promise(async resolve => {
    ...
    if ($.updatePkActivityIdRes && $.updatePkActivityIdRes.length){
      $.newShareCodes = [...$.updatePkActivityIdRes, ...$.temp];
    } else{
      $.newShareCodes = $.temp;
    }
...
function requireConfig() {
  return new Promise(async resolve => {
    ...
    if (!$.updatePkActivityIdRes) {
      $.http.get({url: 'https://purge.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/d_carnivalcityShareCodes.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
      await $.wait(1000)
      $.updatePkActivityIdRes = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/d_carnivalcityShareCodes.json')
    }
    resolve()
  })
}
