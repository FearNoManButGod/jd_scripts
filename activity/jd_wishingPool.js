...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
     ...
     let taskVos= await terutnData("healthyDay_getHomeData", {"appId": "1EFVQwQ", "taskToken": "", "channelId": 1});     ;
     if (safeGet(taskVos)) {
        taskVos = JSON.parse(taskVos);
     }else{
         console.log("获取活动信息失败");
         continue;
     }
     if(taskVos.data.success === 'false'){
     ...
     while (balance >= 500) {
       let res = await terutnData('interact_template_getLotteryResult', {"appId": "1EFVQwQ"})
       res = JSON.parse(res);
       console.log('抽奖结果:成功')
       balance -= 500
     }
...
function terutnData(function_id, body = {}){
...
function doTask(taskToken,taskId,timeout){
    return new Promise(async resolve => {
        if(timeout !== 0){
           let res = await terutnData("harmony_collectScore", {"appId": "1EFVQwQ", "taskToken": taskToken, "taskId": taskId, "actionType": 1});
           res = JSON.parse(res);
           console.log(`领取任务:${res.data.bizMsg} \n 请等待完成`);
           await $.wait(timeout * 1000);
        }
       let data = await terutnData('harmony_collectScore', {"appId": "1EFVQwQ", "taskToken": taskToken, "taskId": taskId, "actionType": 0});
       data = JSON.parse(data);
       if (data.code === 0) {
           try {
             console.log(`任务成功: 获得${data.data.result.score * 1} 余额: ${data.data.result.userScore * 1}`);
           } catch (e) {
             console.log(`任务错误: ${data.data.bizMsg}`)
           }
        }
        await $.wait(1000);
        resolve();
