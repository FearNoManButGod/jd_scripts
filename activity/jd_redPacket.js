...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
	...
	$.HelpRedPacketId = [...($.authorMyShareIds || [])];
    $.redPacketId = [...new Set($.redPacketId)];
	$.redPacketId.forEach(element => {
		if( $.HelpRedPacketId.indexOf(element) == -1){
		  $.HelpRedPacketId.push(element);
		}
	});
    ...
    if (cookiesArr && cookiesArr.length >= 2) {
      ...
      for (let j = 0; j < $.HelpRedPacketId.length && $.canHelp; j++) {
        console.log(`账号 ${$.index} ${$.UserName} 开始给 ${$.HelpRedPacketId[j]} 进行助力`)
        $.max = false;
        await jinli_h5assist($.HelpRedPacketId[j]);
        await $.wait(2000)
        if ($.max) {
          $.HelpRedPacketId.splice(j, 1)
          j--
          continue
        }
      }
...
function requireConfig() {
  return new Promise(async resolve => {
    ...
    let res = await getAuthorShareCode('https://raw.githubusercontent.com/FearNoManButGod/AuthorCode/main/jd_red.json')
	if (!res) {
		$.http.get({url: 'https://purge.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/jd_red.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
		await $.wait(1000)
		res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/FearNoManButGod/AuthorCode@main/jd_red.json')
	}
