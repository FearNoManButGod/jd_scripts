...
All变量适用
————————————————
入口：[ 7.24-8.4 甜蜜七夕 跨越时空中意你 (https://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiSecond/activity?activityId=dz46b2dbbde1914438a47a994cc11c&shareUuid=ad2522798ee7449bb4a97e72cc1463f4)]
...
const $ = new Env('7.24-8.4 甜蜜七夕 跨越时空中意你');
...
opencard_addSku = $.isNode() ? (process.env.opencard_addSku121 ? process.env.opencard_addSku121 : `${opencard_addSku}`) : ($.getdata('opencard_addSku121') ? $.getdata('opencard_addSku121') : `${opencard_addSku}`);
...
opencard = $.isNode() ? (process.env.opencard121 ? process.env.opencard121 : `${opencard}`) : ($.getdata('opencard121') ? $.getdata('opencard121') : `${opencard}`);
...
opencard_draw = $.isNode() ? (process.env.opencard_draw121 ? process.env.opencard_draw121 : opencard_draw) : ($.getdata('opencard_draw121') ? $.getdata('opencard_draw121') : opencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(opencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[opencard121]为"true"')
    }
    if(opencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "f5f67620c39240e49d4a65a6efeb1bcc"
  console.log(`入口:\nhttps://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiSecond/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`)
  let shareUuidArr = [$.shareUuid, "f5f67620c39240e49d4a65a6efeb1bcc", "f5f67620c39240e49d4a65a6efeb1bcc", "f5f67620c39240e49d4a65a6efeb1bcc"]
  ...
  try {
    ...
    if(!$.followSku && !$.outFlag){
      if(opencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[opencard_addSku121]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    }else console.log('如需抽奖请设置环境变量[opencard_draw121]为"3" 3为次数');
  ...
  switch (type) {
    case 'isvObfuscator':
      ...
      case 'accessLogWithAD':
        ...
        let pageurl = `${domain}/dingzhi/jul/qixiSecond/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
      ...
      case 'activityContent':
        url = `${domain}/dingzhi/jul/qixiSecond/activityContent`;
      ...
      case 'drawContent':
        ...
        break;
      case 'checkOpenCard':
        url = `${domain}/dingzhi/jul/qixiSecond/initOpenCard`;
      ...
      case 'startDraw':
        ...
        break;
      case 'followShop':
        url = `${domain}/dingzhi/jul/qixiSecond/saveTask`;
      ...
      case 'getShareRecord':
        ...
        break;
      case '抽奖':
        url = `${domain}/dingzhi/jul/qixiSecond/draw`;
...
function getPostRequest(url, body, method="POST") {
  ...
  if(url.indexOf('https://lzdz1-isv.isvjcloud.com') > -1){
    headers["Referer"] = `https://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiSecond/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
...
function getCk() {
  return new Promise(resolve => {
    let get = {
      url:`https://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiSecond/activity?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
